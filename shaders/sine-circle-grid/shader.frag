...
uniform float sinFreq;
uniform float noiseFreq;
uniform float noiseAmp;
uniform float sinPower;
uniform float d;
uniform int method;
...
void main() {
    ...
    float sinVal = sinAmp * sin(pow(length(p), sinPower)*sinFreq+t);
    float noiseVal = noiseAmp * snoise(vec3(p.xy*noiseFreq, t));
    float n = 0.5+0.5 * sinVal + noiseVal;
    n = clamp(n, 0., 1.);
